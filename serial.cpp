...
/// @section   DESCRIPTION
///            arduinokit serial port classes. Adapted from Serial 
///			   port object for wxWidgets written by Paul Stoffregen
///			   of pjrc.com fame (the guy who made the Teensy).
///
/// 
///
///------------------------------------------------------------------
...
int Serial::Open(const wxString& name)
{
	...
	int bits;
	char *cname = name.char_str();
	port_fd = open(cname, O_RDWR | O_NOCTTY | O_NONBLOCK);
	if (port_fd < 0) {
		if (errno > 0 && errno < sys_nerr)
			error_msg = name + wxT(": ") +
				wxString::FromAscii(sys_errlist[errno]);
		else
			error_msg = name + wxT(": open failed");
		return -1;
	}
	if (ioctl(port_fd, TIOCMGET, &bits) < 0) {
		close(port_fd);
		error_msg = wxString::FromAscii("Unable to query serial port signals");
		return -1;
	}
	bits &= ~(TIOCM_DTR | TIOCM_RTS);
	if (ioctl(port_fd, TIOCMSET, &bits) < 0) {
		close(port_fd);
		error_msg = wxString::FromAscii("Unable to control serial port signals");
		return -1;
	}
	if (tcgetattr(port_fd, &settings_orig) != 0) {
		close(port_fd);
		error_msg = wxString::FromAscii("Unable to query serial port settings (perhaps not a serial port)");
		return -1;
	}
...
#elif defined(MACOSX)
	int bits;
	port_fd = open((const char*)name.mb_str(wxConvUTF8), O_RDWR | O_NOCTTY | O_NONBLOCK);
	if (port_fd < 0) {
...
#if defined(LINUX)
// Taken from Paul Stoffregen's Serial Object:
		...
		if (nameCFstring) {
			if (CFStringGetCString((const __CFString *)nameCFstring,
			   s, sizeof(s), kCFStringEncodingASCII)) {
				list.Add(wxString::FromAscii(s)); //wxString::Format(_T("%s"), s)
			}
